...
from datamodel.search.RscafidiReynagaa_datamodel import RscafidiReynagaaProjectionLink
...
@Deleter(RscafidiReynagaaProjectionLink)
@Getter(RscafidiReynagaaProjectionLink)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(RscafidiReynagaaProjectionLink)
        print "Found ", len(ls), " links to delete."
        for l in ls:
            self.frame.delete(RscafidiReynagaaProjectionLink, l)
